...
func (e floatElement) StringList() []string {
	if e.IsNA() {
		return []string{"NaN"}
	}
	return []string{fmt.Sprintf("%f", e.e)}
}

func (e floatElement) IntList() ([]int, error) {
	if e.IsNA() {
...
func (e floatElement) BoolList() ([]bool, error) {
	if e.IsNA() {
	...
	return nil, fmt.Errorf("can't convert Float \"%v\" to []bool", e.e)
}
